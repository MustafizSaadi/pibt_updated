...
Problem* run(int argc, char *argv[])
{
  ...
  if (envConfig->PTYPE == Param::PROBLEM_TYPE::P_MAPF ||
    ...
    // std::cout<<"pickup nodes: "<<G->getPickup().size()<<"\n";
    // std::cout<<"delivery nodes: "<<G->getDelivery().size()<<"\n";
    // for (int i = 0; i < G->getPickup().size() ; ++i)
    // {
    //   Node* v = G->getPickup()[i];
    //   v->getPos().printPos();
    // }
  ...
  solver->setDirectory(envConfig->field);
  ...
  if (solverConfig->WarshallFloyd) {
    ...
    solver->SaveDataToFile(envConfig->field);
#ifdef OF
    std::cout << "done." << "\n";
#endif
  }
  ...
  if (envConfig->printlog) {
    ...
    std::cout << "solver_time: " << solver->getElapsed() << "\n";
  }
